...
import com.cocktails.IngredientEx
import com.cocktails.model.Ingredient

// TODO: distinguish between DTO and actual Model
public class IngredientsDTOLocalConverter : Mapper<Ingredient, IngredientEx> {
    public override fun toDomain(external: IngredientEx): Ingredient =
        Ingredient(
            id = external.id,
            name = external.name,
    ...
    public override fun toExternal(domain: Ingredient): IngredientEx =
        IngredientEx(
            id = domain.id,
            name = domain.name,
